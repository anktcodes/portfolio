...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof Lenis === 'undefined') {
        console.error('Lenis is not loaded! Check if CDN script is before script.js');
        return;
    }
    ...
    // Initially hide skill cards for animation
    document.querySelectorAll('.skill-card').forEach(card => {
        card.style.opacity = '0';
        card.style.transform = 'scale(0.5)';
    });

    // Scroll-triggered animations for skills
    ...
    const skillsObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                // Add 400ms delay before starting animations
                setTimeout(() => {
    ...
    if (skillsSection) {
        ...
        console.log('Skills observer set up');
    }
    ...
    const projectsObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                console.log('Project card visible, animating...');
    ...
    function observeProjectCards() {
        const projectCards = document.querySelectorAll('.projects-page-grid .project-card');
